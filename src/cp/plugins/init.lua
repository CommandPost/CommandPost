...
local mod = {}
	...
	-- TO-DO: Clean up
	function mod.loadCustomPackage(customPath)

		local path = fs.pathToAbsolute(customPath)
		if not path then
			log.ef("The provided path does not exist: '%s'", path)
			return false
		end
		...
		for i,file in ipairs(files) do
			if file ~= "." and file ~= ".." and file ~= "init.lua" then
				...
				if fs.attributes(filePath).mode == "directory" then
					...
					if attrs and attrs.mode == "file" then
						-- it's a plugin
						--log.df("Plugin: %s", file)

						if string.sub(file, -4) ~= ".lua" then
							file = file .. "/init.lua"
						end

						mod.loadCustom(path .. "/" .. file)
					else
						-- it's a plain folder. Load it as a sub-package.
						--log.df("it's a plain folder. Load it as a sub-package.")
						mod.loadCustomPackage(path .. "/" .. file)
					end
				else
					local name = file:match("(.+)%.lua$")
					if name then
						--log.df("the other one")
						mod.loadCustom(path .. "/" .. name)
					end
				end
			end
		end
	...
	-- TO-DO: Clean up
	function mod.loadCustom(pluginPath)
		...
		local status = dofile(pluginPath)
		--local status, err = pcall(loadfile, pluginPath)
		local plugin = nil
		if status then
			plugin = status
			--plugin = loadfile(pluginPath)
	...
	-- TO-DO: Clean up
	function mod.loadCustomPlugins(customPath)

		-- load the plugins
		log.df("Loading plugin package '%s'", customPath)
		local status, err = pcall(function()
			mod.loadCustomPackage(customPath)
		end)
		if not status then
			log.ef("Error while loading package '%s':\n%s", customPath, hs.inspect(err))
		end
