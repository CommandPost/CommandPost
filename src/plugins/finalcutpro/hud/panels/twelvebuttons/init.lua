...
mod.buttons = json.prop(config.userConfigRootPath, "HUD", "Twelve Buttons.cpHUD", {})

--- plugins.finalcutpro.hud.panels.twelvebuttons.updateInfo() -> none
--- Function
...
function plugin.init(deps, env)
    if fcp:isSupported() then
        ...
        local panel = deps.manager.addPanel({
            priority    = 4,
            id          = "twelvebuttons",
            label       = "12 " .. i18n("buttons"),
            tooltip     = "12 " .. i18n("buttons"),
            image       = imageFromPath(iconFallback(env:pathToAbsolute("/images/twelvebuttons.png"))),
            height      = 120,
            loadedFn    = updateInfo,
        })
        ...
        deps.manager.addHandler("hudtwelvebuttons", controllerCallback)
    end
end
