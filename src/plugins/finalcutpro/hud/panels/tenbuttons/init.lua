...
-- Constant
-- Number of buttons on this panel.
local NUMBER_OF_BUTTONS = 10
...
local function updateInfo()
    ...
    for i=1, NUMBER_OF_BUTTONS do
        ...
        if value then
            ...
            script = script .. [[changeInnerHTMLByID("button]] .. i .. [[", "]] .. i18n("unassigned") .. [[");]] .. "\n"
        end
    end
    if script ~= "" then
...
local function renameButton(id)
    mod._manager.injectScript("renamePrompt('" .. id .. "');")
end
    ...
    if button then
        ...
        if handler then
            if not handler:execute(button.action) then
                log.ef("Failed to execute action: %s", button.actionTitle)
            end
        else
            log.wf("Unable to find handler to execute action: %s", button.actionTitle)
        end
    else
