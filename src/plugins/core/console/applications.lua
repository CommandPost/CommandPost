...
local function modifyNameMap(info, add)
    for _, item in ipairs(info) do
        ...
        if add then
            ...
            -- AppleScript Icon:
            --------------------------------------------------------------------------------
            if string.find(item.kMDItemPath, "%.applescript$") or string.find(item.kMDItemPath, "%.scpt$") then
                icon = imageFromPath(config.bundledPluginsPath .. "/core/console/images/SEScriptDocument.icns")
            end
...
function mod.startSpotlightSearch()
    ...
    mod.spotlight = spotlight.new():queryString([[ (kMDItemContentType = "com.apple.application-bundle") || (kMDItemContentType = "com.apple.systempreference.prefpane")  || (kMDItemContentType = "com.apple.applescript.text")  || (kMDItemContentType = "com.apple.applescript.script") ]])
...
function plugin.init(deps)
    ...
    mod._handler = deps.actionmanager.addHandler("global_applications", "global")
        ...
        :onExecute(function(action)
            if string.find(action["path"], "%.applescript$") or string.find(action["path"], "%.scpt$") then
