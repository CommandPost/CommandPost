...
local mod = {}
	...
	local function controllerCallback(message)
		...
		for i, v in ipairs(mod._uiItems) do
			...
			--------------------------------------------------------------------------------
				local data = v["itemFn"]()
				if data["title"] == title or title == v["customID"] then
	...
	local function generateContent()
		...
		for i, v in ipairs(mod._uiItems) do
			...
			elseif uiType == generate.UI_BUTTON then
				result = result .. "\n" .. generate.button(data, v["customTrigger"], v["customWidth"], v["customID"])
	...
	function mod.init(deps)
		...
		local id 			= "advanced"
		local label 		= "Advanced"
		local image			= image.imageFromName("NSAdvanced")
		local priority		= 2050
		local tooltip		= "Advanced Preferences"
	...
	-- ADD BUTTON:
	--------------------------------------------------------------------------------
	function mod:addButton(priority, itemFn, customWidth, customTrigger, customID)
