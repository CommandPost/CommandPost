...
--- plugins.core.websocket.manager.message-handler.translateMenuActionId(actionId, handlerId) -> string
--- Function
--- Translates an English menu actionId to the current locale if needed.
---
--- Parameters:
---  * actionId - The action ID (e.g., "menu:Modify||Retime||Fast||4x")
---  * handlerId - The handler ID (e.g., "fcpx_menu")
---
--- Returns:
---  * The translated actionId for the current locale, or the original if not a menu action
function mod.translateMenuActionId(actionId, handlerId)
    -- Only process menu actions
    if not actionId or not handlerId or not handlerId:match("_menu$") then
        return actionId
    end

    -- Check if actionId starts with "menu:"
    local menuPrefix = "menu:"
    if not actionId:match("^" .. menuPrefix) then
        return actionId
    end

    -- Extract the menu path from actionId (format: "menu:Path1||Path2||Path3")
    local menuPath = actionId:sub(#menuPrefix + 1)
    ...
    -- Split the path by "||"
    local pathParts = {}
    for part in menuPath:gmatch("[^|]+") do
        if part ~= "" then
    ...
    -- Check if all path parts are ASCII (indicating English path)
    local isEnglish = true
    for _, part in ipairs(pathParts) do
        -- Check if contains non-ASCII characters
        if part:match("[^%w%s%p]") then
            isEnglish = false
            break
        end
    end

    -- If it's already in a non-English language, return as-is
    if not isEnglish then
        log.df("ActionId appears to be in local language, using as-is: %s", actionId)
        return actionId
    end

    -- Try to get the menu handler to translate the path
    local fcp = require "cp.apple.finalcutpro"
    if not fcp or not fcp:isRunning() then
        log.df("Final Cut Pro not running, using English actionId as-is: %s", actionId)
        return actionId
    end

    local currentLocale = fcp:currentLocale()
    if not currentLocale or currentLocale.code == "en" then
        -- Already in English, no translation needed
        return actionId
    end

    -- Get menu titles for both English and current locale
    local menu = fcp.menu
    if not menu then
        log.wf("Unable to access menu, using English actionId as-is: %s", actionId)
        return actionId
    end

    local menuTitles = menu:getMenuTitles({"en", currentLocale})
    if not menuTitles then
        log.wf("Unable to get menu titles, using English actionId as-is: %s", actionId)
        return actionId
    end

    -- Try to translate the path from English to current locale
    local translatedParts = {}
    local currentMenu = menuTitles

    for i, enPart in ipairs(pathParts) do
        local found = false
        for _, item in ipairs(currentMenu) do
            if item["en"] == enPart or item[currentLocale.code] == enPart then
                -- Found matching item
                table.insert(translatedParts, item[currentLocale.code] or enPart)
                if i < #pathParts and item.submenu then
        ...
        if not found then
            -- Can't translate this path, return original
            log.wf("Unable to translate menu path part '%s', using English actionId as-is: %s", enPart, actionId)
            return actionId
        end
    end

    -- Build the translated actionId
    local translatedPath = table.concat(translatedParts, "||")
    local translatedActionId = menuPrefix .. translatedPath
    log.df("Translated actionId from '%s' to '%s'", actionId, translatedActionId)
    return translatedActionId
end
...
function mod.handleCommand(data)
    ...
    -- Translate English menu actionId to current locale if needed
    if actionId and handlerId then
        actionId = mod.translateMenuActionId(actionId, handlerId)
    end
